...
for r in range(max_rounds):
    ...
    if r == 0:
        ...
        print('Round %d, Best (%d), Avg (%.1f min)):' % (r+1, best_round+1, np.mean(times)/60.), 
               flush=True)
    theta, x = simulate_for_sbi(simulator, proposal=proposal, num_simulations=num_sims)
    inference = inference.append_simulations(theta, x)
    density_estimator = inference.train(training_batch_size=num_batch, num_atoms=num_atoms, stop_after_epochs=10)
...
optim = {'summary':inference._summary, 'zs':np.array(zs), 'times':times}
