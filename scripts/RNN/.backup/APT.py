...
class APT(BaseInference):
    ...
    def run(self, n_rounds=1, proposal='gaussian', silent_fail=True, exit_early=False, **kwargs):
        """Run algorithm
        ...
        for r in range(n_rounds):
            ...
            min_val_loss = np.min(log['val_loss'])
            print('Round %r log(p): mean %.2E, min %.2E\n' % (r+1, np.mean(log['val_loss']), min_val_loss))
            logs.append(log)
            trn_datasets.append(trn_data)
            posteriors.append(self.predict(self.obs))

            if last_min_val_loss is not None and r > 1:
                if last_min_val_loss < min_val_loss:
                    print('Exitting SNPE routine after %d rounds.' % (r+1))
                    break
            last_min_val_loss = min_val_loss

            time2 = time.time()
            times.append(time2 - time1)

        return logs, trn_datasets, posteriors, times
