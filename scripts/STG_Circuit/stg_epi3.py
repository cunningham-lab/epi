...
g_synA = Parameter("g_synA", 1, lb=0.1, ub=5.)
g_synB = Parameter("g_synB", 1, lb=3., ub=7.)

# Define model
name = "STG"
parameters = [g_el, g_synA, g_synB]
...
def network_freq(g_el, g_synA, g_synB):
    ...
    def f(x):
        ...
        dVmdt = (1.0 / C_m) * (-I_total + 1e-12*tf.random.normal(I_total.shape, 0., 1.))
