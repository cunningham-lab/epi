...
b = tf.constant(np.array([[1., 1., 1., 1.25]]), dtype=tf.float32)
...
def V1_dr_eps(alpha, inc_val, isn_net=False):
    ...
    def dr(dh):
        ...
        def f1(y):
            drdt = (-y + (tf.nn.relu(tf.matmul(W, y) + b) ** 2.0)) / tau
            return tf.clip_by_value(drdt, -1e4, 1e4)

        def f2(y):
            drdt = (-y + (tf.nn.relu(tf.matmul(W, y) + h) ** 2.0)) / tau
            return tf.clip_by_value(drdt, -1e4, 1e4)
