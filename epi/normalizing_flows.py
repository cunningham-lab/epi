...
class Arch:
    ...
    def _set_num_layers(self, num_layers):
        if (type(num_layers) is not int):
            ...
            raise ValueError('Arch num_layers arg %d must be greater than 0.' % num_layers)
...
class CouplingArch(Arch):
    def __init__(self, D, num_couplings, num_layers, num_units, post_affine=True):
        ...
        self.q0 = tfd.MultivariateNormalDiag(loc=self.D*[0.])
    ...
    def _set_num_couplings(self, num_couplings):
        if (type(num_couplings) is not int):
            raise TypeError(format_type_err_msg(self, 'num_couplings', num_couplings, int))
        elif (num_couplings < 1):
            raise ValueError('CouplingArch num_couplings %d must be greater than 0.' % num_couplings)
    ...
    def _set_num_units(self, num_units):
        if (type(num_units) is not int):
            ...
            raise ValueError('CouplingArch num_units %d must be greater than 0.' % num_units)
        self.num_units = num_units
        
    def to_string(self,):
        arch_string = 'NVP_%dC_%dL_%dU' % (self.num_couplings, 
                                           self.num_layers, 
                                           self.num_units)
        if (self.post_affine):
            arch_string += '_PA'
