...
Strong anticorrelation between $p_P$ and $\lambda_{\text{all}}$ (Fig. \ref{fig:SC_Eigs}B) suggests that accuracy in the Pro task is greater when all parameters decrease along the strongly correlated posterior.
...
In contrast, intermediate parameter values result in greater $p_A$ than extremal parameter vaues (Fig. \ref{fig:SC_EPI2}).
While $p_P$ is well predicted by the remaining Schur eigenvalues (correlated with $\lambda_{\text{task}}$ and $\lambda_{\text{diag}}$ and anticorrelated with $\lambda_{\text{side}}$), $p_A$ is not well predicted by any of them (Fig. \ref{fig:SC_Eigs}B).
