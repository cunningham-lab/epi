...
Some parameters were predictive of accuracy while others were not (Fig. \ref{fig:SX1}), and often had different effects on $p_P$ and $p_A$.
...
These same connectivity variables are anticorrelated with Anti accuracy across the EPI posterior.
By visualizing these four variables together with $p_A$ (Fig. \ref{fig:SX2}B), we see that low $\lambda_{\text{task}}$ and $\lambda_{\text{diag}}$ producing strong Anti accuracy also have high $\lambda_{\text{side}}$ and $\lambda_{\text{all}}$.
...
Even though $p_P$ and $p_A$ are anticorrelated in the EPI posterior ($r=-0.15$, $p=3.68 \times 10^-12$), greater $p_P$ and $p_A$ both result in decreased resiliency to delay perturbation in the Anti task.
Ultimately, lower $\lambda_{\text{side}}$ and $\lambda_{\text{all}}$ and greater $\lambda_{\text{task}}$ produce networks more robust to delay perturbation.
...
\begin{figure}
\begin{center}
\includegraphics[scale=0.6]{figs/figSX3.pdf}
\end{center}
\caption{\small 
...
\begin{figure}
\begin{center}
\includegraphics[scale=0.75]{figs/figSX4.pdf}
\end{center}
\caption{\small Scatters of the effect of delay period inactivation in each task with task accuracy.  Plots are shown at an opto strength of 0.8.
}
\label{fig:SX4}
\end{figure}
