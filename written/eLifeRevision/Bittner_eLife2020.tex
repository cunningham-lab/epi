...
A key process in neuroscientific modeling is then to identify the model parameters consistent with observed phenomena: to solve the inverse problem.
While statistical inference has proven effective on a broad variety of neuroscientific datasets, we clarify an important incongruity between theoretical approaches to neuroscience and this probabilistic methodology.
Theoretical neuroscience is focused on computational properties and how they emerge from biological mechanisms, rather than noisy experimental datasets and their quantified structure.
In this work, we present a novel technique to directly infer circuit model parameters producing these computational properties.
This method tailors deep inference, the use of deep neural networks for statistical inference, to the nature of theoretical inverse problems, enabling scaling to high dimensions and considerable technical simplifications.
With this method, we bring deep inference to bear on important questions in theoretical neuroscience, and demonstrate the broad range of insightful analyses this approach allows.
First, we emphasize the general applicability of this approach by inferring channel conductance parameters that produce a distinctive spiking frequency in a biophysical model of the stomatogastric ganglion.
Then, in a model of primary visual cortex with multiple neuron-types, where  analysis becomes untenable as more neuron-types are included, we use this technique to discover how noise properties govern excitatory population variability.
Next, in a model of superior colliculus, we identify and characterize two distinct regimes of connectivity that facilitate switching between opposite tasks throughout interleaved trials. 
Finally, we scale inference to 1,000-dimensional parameter spaces of RNN connectivity that exhibit stable, yet amplified responses.
These analyses illustrate how we can further leverage the power of deep learning towards solving inverse problems in theoretical neuroscience.
...
Alternatively, we can gain insight into these complex models by identifying the distribution of parameters that produce a specific computational property.
...
One preferred formalism for parameter identification is statistical inference, which has been used to great success in neuroscience through the stipulation of statistical generative models \cite{kass2001spike, brown1998statistical, paninski2004maximum, truccolo2005point, schneidman2006weak, druckmann2007novel, turner2007maximum, byron2009gaussian, macke2011empirical, park2011bayesian, granot2013stimulus, latimer2015single, lakshminarasimhan2018dynamic, duncker2019learning, ladenbauer2019inferring} (see review, \cite{paninski2018neural}). 
Recent work has used variational autoencoders (VAEs) \cite{kingma2013auto, rezende2014stochastic} to interrogate hidden states in models of both cortical population activity \cite{gao2016linear, zhao2017recursive, barello2018sparse, pandarinath2018inferring} and animal behavior \cite{wiltschko2015mapping, johnson2016composing, batty2019behavenet}, thus expanding the domain of neural data sets amenable to statistical modeling.
However, most neural circuit models in theoretical neuroscience are noisy systems of differential equations that can only be sampled or realized through forward simulation; they lack the explicit likelihood necessary for statistical inference.
...
However, as we detail, all of these approaches require good datasets for the scientific question at hand.
...
This work seeks to clarify an important incongruity between theoretical approaches to neuroscience and existing statistical inference methodology.
In theoretical neuroscience, we are concerned with the computational properties -- the \emph{emergent phenomena} -- of our models \cite{hopfield1982neural, sompolinsky1988chaos, tsodyks1997paradoxical, wong2006recurrent}, not noisy observed datasets \cite{paninski2018neural}.
...
Theorists are therefore barred from using the probabilistic modeling toolkit for science, unless they reformulate their inverse problem to fit an evidence accumulation framework.
...
EPI fixates on the inverse problem by constraining the predictions of the inferred parameter distribution to produce the emergent property exactly.
Conditioning on an emergent property requires a variant of deep probabilistic inference methods, which we have previously introduced \cite{loaiza2017maximum}.
By using deep probability distributions EPI flexibly captures parametric structure in mechanistic models.
This technique is designed to identify the full parameter space producing an emergent property and facilitate the seemless structural analysis of the inferred parameter distribution.
Thus, EPI provides neuroscientists with an efficient, versatile probabilistic modeling toolkit designed explicitly for theoretical inverse problems.

Equipped with this method, we bring deep inference to bear on theoretical neuroscience to an unprecedented extent.
Throughout this work, we showcase the capabilities of EPI on four neural circuit models across ranges of biological realism, neural system function, and network scale.
First, we show EPI's ability to capture subtle, nonlinear parametric structure in a stomatogastric ganglion subcircuit model \cite{gutierrez2013multiple}.
In a model of primary visual cortex \cite{palmigiano2020structure}, we show how to gain insight from multiple inferred distributions.
Next, we used EPI to idenitfy and structurally characterize multiple parametric regimes of superior colliculus activity in a model of task switching \cite{duan2018collicular}.
Finally, we emphasize the superior scalability of EPI compared to other LFI techniques by inferring high-dimensional distributions of RNN connectivities that exhibit amplified, yet stable responses -- a hallmark of cortical sensory systems \cite{more, bondanelli2020coding}.

Most importantly of all in this work, we present novel theories of neural computation borne from EPI analysis.
We identified an unknown parametric rule of variability with respect to inhibitory neuron type in a V1 model, where analytic techniques became untenable.
Furthermore, we identified multiple regimes of SC connectivity which confer rapid task switching, and used the structural analytic tools of EPI to gain a mechanistic understanding of circuit responses.
These valuable theoretical insights illustrate the value of deep inference for the interrogation of neural circuit models.
...
Third, parameter analyses ensue: brute-force parameter sweeps, ABC sampling \cite{liepe2014framework}, and sensitivity analyses \cite{raue2009structural} are all routinely used to reason about what parameter configurations lead to an emergent property.
In this last step lies the opportunity for a precise quantification of the emergent property as a statistical feature of the model.
Once we have such a methodology, we can infer a probability distribution over parameter configurations that produce this emergent property.
...
This distribution -- represented for clarity as samples from the distribution -- is then the most random parameter distribution producing the emergent property. 
...
Mathematically, we achieve this with two constraints: by setting the mean hub frequency of model generated activity to 0.55Hz.
...
\end{equation}
and requiring that the variance around this mean is small
...
\end{equation}
This level of variance was chosen to be low enough to exclude the fast and slow frequencies of the two populations, but large enough to allow structural examination of the compatible parameter space.
...
The choice of $\bm{\sigma}^2$ predicates the degree of variability in around mean $\bm{\mu}$ that is consistent with the emergent property.
...
This deep distribution is represented in Figure \ref{fig:STG}D (see Section \ref{methods_EPI}).
Many distributions in $\mathcal{Q}$ will respect the emergent property constraints, so we choose a normative selection principle imposing no additional structure beyond the production of the emergent property \cite{jaynes1957information, elsayed2017structure, loaiza2017maximum, savin2017maximum}, which is the same normative principle of Bayesian inference (see Section \ref{methods_VI}).
...
The probabilities of the distribution inferred from EPI are the densities of these parameters in the most random distribution producing the emergent property.
While existing approaches to probabilistic structural identifiability analysis use the lens of evidence accumulation \cite{raue2009structural, gonccalves2019training}, sensitivity and robustness of parameter space dimensions with respect to emergent phenomena can be directly quantified with EPI.
Sensitivity quantifications are measured by the second order derivative of EPI probability along the parameteric dimension of interest: to what extent is the emergent property maintained or diminished along this dimension?
Once an EPI distribution has been inferred, this second order derivative requires trivial computation (as long as the correct architecture class is chosen, see Section \ref{methods_NF}).
Equipped with this method, we may examine structure in the resulting parameter distributions or make comparisons between distributions conditioned at different levels of the same emergent property statistic.
In Sections \ref{results_V1} and \ref{results_SC}, we prove out the value of EPI by using these techniques to investigate and produce novel scientific insight.
...
Transient amplification is a hallmark of neural activity throughout cortex, and is often thought to be intrinisically generated by recurrent connectivity in the responding cortical area \cite{murphy2009balanced, hennequin2014optimal, bondanelli2019population}.
...
\end{center}
\caption{\small 
\textbf{A}. Wall time of EPI (blue), SNPE (orange), and SMC-ABC (green) to converge on RNN connectivites producing stable amplification.  
...
\textbf{D}. Simulations of networks inferred by each method ($\tau=100ms$).  Each trace (15 per algorithm) corresponds to simulation of one $z$.  (Below) Ratio of obtained samples producing stable amplification, monotnoic decay, and instability.
}
\label{fig:LRRNN}
\end{figure}
...
SMC-ABC is a rejection sampling approach that uses sophisticated techniques to improve efficiency, and SNPE approximates posteriors with deep probability distributions using a two-network architecture (see Section \ref{methods_related}).
Unlike EPI, these statistical inference techniques do not control the mean or variance of the predictive distribution, and these predictions of the inferred posteriors are typically affected by model characteristics (e.g. $N$ and $g$, Fig. \ref{fig:RNN1}).
...
For the dimensionalities in which SMC-ABC is tractable, the inferred parameters always exhibit stable amplification  (Fig. \ref{fig:LRRNN}C, green), but are concentrated and offset from $\mathbf{x}_0$.
When using SNPE the inferred parameters are highly concentrated at some RNN sizes and widely varied in others (Fig. \ref{fig:LRRNN}C, orange).
We see these properties reflected in simulations from the inferred distributions: EPI produces a consistent variety of stable amplified activity norms $|r(t)|$, SMC produces a limited variety in respones, and the changing variety of responses from SNPE emphasizes the control of EPI on parameter predictions.

From this analysis, we see that deep inference techniques EPI and SNPE are far more amenable to high dimensional parameter distributions than rejection sampling techniques like SMC-ABC, and have the additional benefit of fast sampling after optimization.
EPI outperforms SNPE in high dimensions by leveraging gradient information (from $\nabla_\mathbf{z} f(\mathbf{x}; \mathbf{z}) = \nabla_\mathbf{z} [\text{real}(\lambda_1), \lambda_1^s]^\top$) on each optimization iteration and constantly adapting the approximation $q_{\bm{\theta}}(\mathbf{z})$ to the parameter distribution.
While EPI can be used for scientific insight into RNNs, this analysis shows that when $\nabla_\mathbf{z} f(\mathbf{x}; \mathbf{z})$ is tractable, EPI can be used to efficiently infer high dimensional parameter distributions in mechanistic models of neural computation.
...
\tau \dot{\mathbf{x}} = -\mathbf{x} + W\mathbf{x}.
\end{equation}
In this analysis, we infer connectivity parameterizations $\mathbf{z} = \left[\mathbf{u}_1^\top, \mathbf{u}_2^\top, \mathbf{v}_1^\top, \mathbf{v}_2^\top \right]^\top \in \left[-1, 1 \right]^{(4N)}$   that produce stable amplification.
...
We define the emergent property of stable amplification with means of these eigenvalues (0.5 and 1.5, respectively) that satisfy these conditions and variances ($0.25^2$) about those means such that samples rarely violate the eigenvalue constraints.
As a maximum entropy distribution, this is written as
\begin{equation} 
...
We used an augmented Lagrangian coefficient of $c_0 = 10^{3}$, a batch size $n=200$, $\beta=4$, and chose to use $500$ iterations per augmented Lagrangian epoch.
...
\textbf{How we did EPI} \\

\textbf{How we did SNPE} \\
...
\end{center}
\caption{\small (RNN1): Model characteristics affect predictions of posteriors inferred by SNPE, while predictions of parameters inferred by EPI remain fixed.
...
\textbf{D}. Adherance to EPI constraints. Same conventions as B.
}
\label{fig:RNN1}
\end{figure}
