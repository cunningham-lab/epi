...
Alternatively, Bayesian inference can be run to obtain model parameters likely to produce some observations, and local sensitivity analyses can be performed at individual parameter choices of the obtained posterior.
...
Therefore, the most popular approaches to the inverse problem have been likelihood-free methods namely approximate Bayesian computation (ABC) \cite{liepe2014framework}, in which a set of model parameters analogous to a posterior is obtained via simulation and rejection.
...
First, principled Bayesian approaches using MCMC or VAEs do not work absent a tractable likelihood function, which theoretical models of neural computation generally lack.
The parameter sets obtained from likelihood-free ABC lack a formalized link to Bayesian inference (except for the theoretical 0-distance scenario), lack parameter probabilities, and only confer sensitivity analyses of an alternative likelihood to the simulator-defined likelihood of ABC \cite{raue2009structural}.
Second is the undesirable trade-off between the flexibility and tractability of the approximated posterior distribution.
While sampling-based approaches like ABC and MCMC can produce flexible posterior approximations, they must be run continually for increasing samples.
While VAE approaches result in tractable posterior sampling and sensitivity measurements post-optimization, existing approaches have relied on simplified classes of distributions, which restrict the flexibility of the posterior approximation.
And third, the posterior predictive distribution of all Bayesian approaches to the inverse problem are left unconstrained.  
...
However, unconstrained posterior predictive distributions introduce a conceptual degree of freedom to the inverse problem that may be unnecessary and undesirable given the scientific motivation.
