...
Appoximate Bayesian computation (ABC) \cite{beaumont2002approximate} is a rejection sampling technique for obtaining sets of parametes $\mathbf{z}$ that produce activity $\mathbf{x}$ close to some observed data $\mathbf{x}_0$.
...
To compare the efficiency of these algorithms, we start with the basic principle that you want z to produce x.
In EPI this is blah with varince.
In SNPE this is implicitly a goal of posterior inference.
In SMC this is explicitly a goal, accuracy controlled by epsilon.
Therefore we measure time to convergence and simulation counts as x 
...
The hyperparameters of SNPE have a serous affect.
Need to check whether more aggressive might lead to converge.
Nsims
Natoms
Nsims at 100.
