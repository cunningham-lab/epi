...
The optimization is complete when the sampled models with parameters $\mathbf{z} \sim q_{\bm{\theta}}(z \mid \mathcal{X})$ produce activity consistent with the specified emergent property (Fig. \ref{fig:STG1}).
...
\vspace{-1cm}
\begin{center}
\includegraphics[scale=0.2]{figures/fig2/fig2.pdf}
\end{center}
\vspace{-1cm}
...
\begin{figure}
\begin{center}
\includegraphics[scale=0.48]{figures/fig3/fig3.pdf}
\end{center}
\caption{\small 
...
\begin{equation}
 ...
 and dynamics
 \begin{equation}
 \tau \frac{d\mathbf{x}}{dt} = -\mathbf{x} + W\mathbf{x}
 \end{equation}
...
\begin{figure}
\begin{center}
\includegraphics[scale=1.4]{figures/fig4/fig4.pdf}
\end{center}
\caption{\small 
...
Normalizing flow architectures for deep probability distributions used in EPI are specified by the number of coupling stages, neural network layers and units per layer for conditioning functions.
...
Deep neural network weights and biases $\bm{\theta}$ of the deep probability distribution are optimized according to Equation \ref{eq:AL} using the Adam optimizer with learning rate $10^{-3}$ \cite{kingma2014adam}.
...
To assess whether the EPI distribution $q_{\bm{\theta}}(\mathbf{z})$ produces the emergent property, we defined a hypothesis testing convergence criteria.  
...
To gain intuition for EPI optimization, consider a two-dimensional linear dynamical system (2D LDS) model (Fig. \ref{fig:LDS1}A):
\begin{equation} 
...
\end{equation}
$\tau = 1$s, and $\mathbf{z} = [a_{1,1}, a_{1,2}, a_{2,1}, a_{2,2}]^\top$.
\begin{figure}
\begin{center}
\includegraphics[scale=0.7]{figures/figLDS1/figLDS1.pdf}
\end{center}
...
B. The EPI distribution for a two-dimensional linear dynamical system with $\tau=1$ that produces oscillations around 1Hz.  
Inset show distribution of eigenvalues across the EPI posterior.
C. (Left) Entropy throughout the optimization with different $c_0$.
Individual optimizations were run with the same architecture and random seed.
Norm of emergent property statistic errors throughout optimization.  
}
\label{fig:LDS1}
\end{figure}
 
To learn the distribution of real entries of $A$ that produce a band of oscillating systems around 1Hz, we formalized this emergent property as $\text{real}(\lambda_1)$ having mean zero and the oscillation frequency $2 \pi \text{imag}(\lambda_1)$ having mean $\omega = 1$ Hz.
Thus, our statistics vector $f(\mathbf{x}; \mathbf{z})$ for this emergent property is two dimensional, and after specifying the variance constraint is
\begin{equation}
\begin{split}
\mathcal{X} ~~:~~ \mathbb{E}_{\mathbf{z}}\begin{bmatrix} \text{real}(\lambda_1 (\mathbf{x}; \mathbf{z})) \\ \text{imag}(\lambda_1 (\mathbf{x}; \mathbf{z})) \end{bmatrix}  &~~=~~  \begin{bmatrix} 0 \\ 2 \pi \omega \end{bmatrix}  \\ 
 \text{Var}_{\mathbf{z}}\begin{bmatrix} \text{real}(\lambda_1 (\mathbf{x}; \mathbf{z}))\\ \text{imag}(\lambda_1 (\mathbf{x}; \mathbf{z})) \end{bmatrix}  &~~=~~  \begin{bmatrix} 0.5^2 \\ (\frac{\pi}{5})^2 \end{bmatrix}.
\end{split}
\end{equation}

Notation of mean and variance taken with respect to $\mathbf{x}$ is omitted, since this model is deterministic, and EPI generalizes naturally to this scenario.
 
 \begin{figure}
\begin{center}
\includegraphics[scale=0.7]{figures/figLDS2/figLDS2.pdf}
\end{center}
\caption{\small (LDS2): A. Probability contours in the $a_{1,1}$-$a_{2,2}$ plane were derived from the relationship to emergent property statistic of growth/decay factor $\text{real}(\lambda_1)$. 
B. Probability contours in the $a_{1,2}$-$a_{2,1}$ plane were derived from the emergent property statistic of oscillation frequency $2\pi \text{imag}(\lambda_1)$.}
\label{fig:LDS2}
\end{figure}
...
Importantly, even though $f(\mathbf{x}; \mathbf{z})$ is analytically available and does not require simulation, we cannot derive the distribution $q^*_{\bm{\theta}}(z \mid \mathcal{X})$ directly.  
...
Instead, we can use EPI to approximate this distribution (Fig. \ref{fig:LDS1}B). 
We used a real-NVP normalizing flow architecture with three masks, two neural network layers of 50 units per mask, mapped onto a support of $z_i \in \left[-10, 10 \right]$. (see Section \ref{methods_NF}).

The effect of $c_0$ on the EPI optimization is shown in Figure \ref{fig:LDS1}C.
Each EPI optimization was run for enough augmented Lagrangian epochs until convergence, with 1,000 iterations per epoch.  
We see that low values of $c_0$ (e.g. $10^{-5}$) result in fast entropic growth to the uniform distribution at the outset of optimization. 
We know this, since the dashed black line indicates the maximum entropy of a distribution with the given bounds. 
Initial entropic growth can be desireable based on the application, however one should expect a greater number of epochs until convergence compared to a greater value of $c_0$ (e.g. $10^{-2}$).
...
In the $a_1$-$a_4$ plane, the black line at $\text{real}(\lambda_1) = \frac{a_{1,1} + a_{2,2}}{2} = 0$, dotted black line at
the standard deviation $\text{real}(\lambda_1) = \frac{a_{1,1} + a_{2,2}}{2} \pm 0.5$, and the dotted gray line at twice the standard deviation
$\text{real}(\lambda_1) = \frac{a_{1,1} + a_{2,2}}{2} \pm 1$ follow the contour of probability density of the samples (Fig. \ref{fig:LDS2}A). 
...
\begin{figure}
\begin{center}
\includegraphics[scale=0.7]{figures/figLDS3/figLDS3.pdf}
\end{center}
\caption{\small (LDS3): Sampled dynamical systems $\mathbf{z} \sim q_{\bm{\theta}}(\mathbf{z} \mid \mathcal{X})$ and their simulated activity from $\mathbf{x}(0) = [\frac{\sqrt{2}}{2}, -\frac{\sqrt{2}}{2}]$ colored by log probability. 
A. Each dimension of the simulated trajectories throughout time.  
B  The simulated trajectories in phase space.}
\label{fig:LDS3}
\end{figure}

To explain the bimodality of the EPI distribution, we examined the imaginary component of $\lambda_1$.  When $\text{real}(\lambda_1) = \frac{a_{1,1} + a_{2,2}}{2} = 0$, we have
\begin{equation}
...
When $\tau=1$ and $a_{1,1} a_{2,2} > a_{1,2} a_{2,1}$ (center of distribution above), we have the following equation for the other two dimensions:
\begin{equation}
\text{imag}(\lambda_1)^2 = a_{1,1} a_{2,2} - a_{1,2} a_{2,1}
\end{equation}
Since we constrained $\mathbb{E}_{\mathbf{z} \sim q_{\bm{\theta}}}\left[\text{imag}(\lambda)\right] = 2 \pi$ (with $\omega=1$), we can plot contours of the equation $\text{imag}(\lambda_1)^2 = a_{1,1} a_{2,2} - a_{1,2} a_{2,1} = (2 \pi)^2$ assuming $a_{1,1} a_{2,2} = \mathbb{E}_z\left[a_{1,1} a_{2,2}\right]$ (Fig. \ref{fig:LDS2}B). 
...
\begin{figure}
\begin{center}
\includegraphics[scale=0.85]{figures/figSC1/figSC1.pdf}
\end{center}
...
\begin{figure}
\begin{center}
\includegraphics[scale=1.3]{figures/figSC2/figSC2.pdf}
\end{center}
\caption{\small (SC2):  
...
\begin{figure}
\begin{center}
\includegraphics[scale=0.6]{figures/figSC3/figSC3.pdf}
\end{center}
\caption{\small (SC3): 
...
\begin{figure}
\begin{center}
\includegraphics[scale=0.75]{figures/figSC4/figSC4.pdf}
\end{center}
