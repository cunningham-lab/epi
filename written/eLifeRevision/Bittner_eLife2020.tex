...
We present a novel technique designed to infer parameter distributions that produce mathematically defined emergent properties of computation.
We detail how the parameter distributions of theoretical neuroscience problems are better defined by the emergent properties they produce, rather than conditioned upon artificial datasets.
This approach is not only well-motivated, but allows technical simplifications to statistical inference in mechanistic models and scaling to high dimensions.
...
First, we show a worked example analyzing parametric sensitivity in the stomatogastric ganglion, emphasizing the general applicability of this approach to complex models with complex emergent properties.
Then, in a model of primary visual cortex where mathematical analysis becomes untenable, we identify a nonlinear manifold revealing the rule of parametric degeneracy with respect to variability.
Next, we gain a mechanistic understanding of how superior colliculus connectivity produces rapid task switching in multiple parametric regimes.
Finally, we scale inference of RNN connectivity exhibiting stable amplification to 1,000-dimensional parameter spaces.
