...
\begin{equation}\label{eq:EP}
\begin{split}
\mathcal{X} ~~:~~ \mathbb{E}_{\mathbf{z},\mathbf{x}}\left[T(\mathbf{x}; \mathbf{z})\right] &~~\triangleq~~  \mathbb{E}_{\mathbf{z},\mathbf{x}}\begin{bmatrix} \omega_{\text{f1}}(\mathbf{x}; \mathbf{z}) \\ \omega_{\text{f2}}(\mathbf{x}; \mathbf{z}) \\ \vdots \end{bmatrix}  ~~=~~  \begin{bmatrix} 0.53 \\ 0.53 \\ \vdots \end{bmatrix} ~~\triangleq~~ \bm{\mu}  \\ 
 \text{Var}_{\mathbf{z},\mathbf{x}}\left[T(\mathbf{x}; \mathbf{z})\right] &~~\triangleq~~ \text{Var}_{\mathbf{z},\mathbf{x}}\begin{bmatrix} \omega_{\text{f1}}(\mathbf{x}; \mathbf{z}) \\ \omega_{\text{f2}}(\mathbf{x}; \mathbf{z}) \\ \vdots \end{bmatrix}  ~~=~~  \begin{bmatrix} 0.025^2 \\ 0.025^2 \\ \vdots \end{bmatrix} ~~\triangleq~~ \bm{\sigma}^2 .
\end{split}
\end{equation}
...
&\text{  s.t.  } ~\mathcal{X}:~~ \mathbb{E}_{\mathbf{z},\mathbf{x}}\left[T(\mathbf{x}; \mathbf{z})\right] = \bm{\mu}, \text{Var}_{\mathbf{z},\mathbf{x}}\left[T(\mathbf{x}; \mathbf{z})\right] = \bm{\sigma}^2 \\
\end{split}
\end{equation}
...
In EPI, emergent properties are defined as statistics having a fixed mean and variance as in Equation \ref{eq:EP}
 \begin{equation}
...
Since $q_{\bm{\theta}}(z)$ failed to produce enough samples yielding $\omega_{\text{f1}}(x)$ less than $0.53$Hz, the convergence criteria were not satisfied after the third epoch at 7,500 iterations.    
...
The Hessian of a probability model is the second order gradient of the log probability density $\log q_{\bm{\theta}}(z)$ with respect to the parameters $z$:  $\frac{\partial^2 \log q_{\bm{\theta}}(z)}{\partial z \partial z^\top}$.  
...
In the final analysis for this model, we sweep the input one neuron at a time away from the mode of each inferred distributions $dh^* = z^* = \argmax_{z} \log q_{\bm{\theta}}(z \mid \mathcal{B}(\alpha, 0.1)$.
