...
A key advantage of EPI over established Bayesian inference is that the predictions made my the inferred distribution are constrained to produce the specified emergent property.
...
\textbf{B}. Responses of the deterministic smodel ($\bm{\epsilon}=\bm{0}$) to varying contrasts.  The response at 50\% contrast (dots) is the focus of our analysis.
\textbf{C}. Paradoxical response of the stochastic model to a small increase in input to the P-population.  
\textbf{D}. EPI posterior of noise parameteres $\mathbf{z}$ conditioned on realistic E-population Fano factors.
...
The slow noise of the SSSN is an an Ornstein-Uhlenbeck process
\begin{equation}
...
The model responds to the side with greater Pro neuron activation; e.g. the reponse is left if $x_{LP} > x_{RP}$ at the end of the trial.
...
\textbf{E}. Difference in performance of each task during inactivation.  Inactivation level ``opto strength" scales from no inactivation (0) to full inactivation (1).  We compare delay period inactivation $1.2 < t < 1.5$ (blue), choice period inactivation $1.5 < t$ (red), and total inactivation $0 \leq t \leq 1.8$ (purple).
...
This shows that strong task representations and hemispheral dominance in the dynamics result in better execution of the Pro task.
...
Thus, stronger hemispheral dominance, relaxed task and diag mode dynamics, and slower circuit-wide decay result in greater Anti accuracy.

In agreement with experimental results form Duan et al., we found that inactivation above nominal strength during the delay period consistently decreased performance in the Anti task, but had no consistent effect on the Pro task (Fig. \ref{fig:SC}E) e.g. (Fig. \ref{fig:SC}D, bottom).
...
This posterior revealed the mechanisms leading to greater accuarcy in each task as well as those increasing resiliency to perturbation in the Anti task.
...
Even so, we are not so much scientifically interested in reasoning about how $\mathbf{z}$ governs all of $\mathbf{x}$, but rather specific phenomena that are a function of the data $f(\mathbf{x}; \mathbf{z})$.
...
Inverse problems across scientific fields have been approached in a variety of ways.
The goals of such approaches are generally to a.) identify parametric sensitivities and manifolds of degeneracy with respect to some model output (structural identifiability analysis), and to reason about the parameters that are most probable to have produced a model output (statistical inference).
While much research in computational neuroscience has focused on optimizing neural architectures to process information and accomplish tasks \cite{richards2019deep}, structure in parameter space of the set of optimized solutions is rarely discussed and lacks a probabilistic treatment.

To understand sensitivity around a point, one can measure the Jacobian. One approach that scales well is EAR \cite{karlsson2012efficient}.
...
Casting identifiability as a statistical estimation problem, the profile likeilhood can assess via iterated optimization while holding parameters fixed \cite{raue2009structural}.
An exciting recent method is capable of recovering the functional form of such unidentifiabilities away from a point \cite{raman2017delineating}.
Global structual non-identifiabilities can be found for models with polynomial or rational dynamics equations using DAISY \cite{saccomani2003parameter}.
...
optimization with respect to entropy, and to confer immediate probability evaluations after optimization.

\subsubsection{Augmented Lagrangian optimization}\label{methods_AL_opt}
Since we are optimizing parameters $\bm{\theta}$ of our deep probability distribution with respect to the entropy $H(q_{\bm{\theta}}(\mathbf{z}))$, we must take gradients with respect to the log probability density of samples from the deep probability distribution.  
Entropy of $q_{\bm{\theta}}(\mathbf{z})$ can be expressed using the reparameterization trick as an expectation of the negative log density of parameter samples $\mathbf{z}$ over the randomness in the parameterless initial distribution $q_0(\mathbf{z}_0$):
\begin{equation}
H(q_{\bm{\theta}}(\mathbf{z})) = \int - q_{\bm{\theta}}(\mathbf{z}) \log(q_{\bm{\theta}}(\mathbf{z})) d\mathbf{z} = \mathbb{E}_{\mathbf{z} \sim q_{\bm{\theta}}}\left[-\log(q_{\bm{\theta}}(\mathbf{z})) \right] = \mathbb{E}_{\mathbf{z}_0 \sim q_0}\left[-\log(q_{\bm{\theta}}(g_{\bm{\theta}}(\mathbf{z}_0))) \right].
\end{equation}
...
\begin{figure}
\begin{center}
\includegraphics[scale=0.58]{figures/figLDS1/figLDS1.pdf}
\end{center}
\begin{flushleft}
...
Since the E-population of this network increases exponentially in the abscense of  recurrent inhibitory feedback, we may also observe a paradoxical effect in the inhibitory populations (which is present in E-I networks).
...
Fano factor is calculated as the temporal variance divided by the temporal mean following some time $t_{ss}$ following dynamical evolution from the initial state at $\mathbf{x}(t=0)$.
...
\end{center}
\caption{\small (SC3): 
A. Connectitivy eigenvalues of EPI parameter distribution colored by Pro task accuracy. 
