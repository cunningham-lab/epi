...
And, we show that EPI is the only method that has explicit control over the predictions of its inferred distribution (Section 3.3, Figure 2C-D).
...
Even with a high degree of biophysical realism and expensive emergent property gradients, EPI was run successfully on intermediate hub frequency in a 5-neuron subcircuit model of the STG (Section ref{results\_motivating}).
...
Amortization is not possible in EPI, since EPI learns an exponential family distribution parameterized by its mean (see Section ref{methods\_ME\_EF})."
\end{displayquote}
...
\begin{displayquote}
Lines X-Y\\
``No matter the number of neurons, EPI always produces connectivity distributions with mean and variance of $\text{real}(\lambda_1)$ and $\lambda_1^s$ according to $\mathcal{X}$ (Fig. 2C, blue).
For the dimensionalities in which SMC-ABC is tractable, the inferred parameters are concentrated and offset from $\mathbf{x}_0$ (Fig. 2C, green).
When using SNPE, the predictions of the inferred parameters are highly concentrated at some RNN sizes and widely varied in others (Fig. 2C, orange).
...
We show that with each neuron-type population added to this E-I model, calculations of excitatory variability with respect to noise parameters become unruly and challenging to work with (see Section ref{methods\_V1\_complexity}).
...
\begin{displayquote}
Lines X-Y \\
``A subcircuit model of the STG (Guttierez et al. 2013) is shown schematically in Figure 1A.
...
In the current manuscript, we make sure to explain all choices of the emergent property means and variances, and why our conclusions are not spuriously affected by ambiguity in these choices.

Quote <All instances of emergent property explanation>
...
Thank you for the comments… (*refer to exact lines of examples of this.) \\
