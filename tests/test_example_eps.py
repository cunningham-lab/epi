...
def test_linear2D_freq():
    N = 100
    a11s = np.random.normal(0., 5., (N,)).astype(DTYPE)
    a12s = np.random.normal(0., 5., (N,)).astype(DTYPE)
    a21s = np.random.normal(0., 5., (N,)).astype(DTYPE)
    a22s = np.random.normal(0., 5., (N,)).astype(DTYPE)
    ...
    T_x_np = np.stack((alphas, np.square(alphas - mean_alphas), omegas, np.square(omegas - mean_omegas)), axis=1)
